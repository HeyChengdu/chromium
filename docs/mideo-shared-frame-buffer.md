...
`trace_shared_frame.py` 使用四个独立 Browser，每个在 2560×1440 先预热 10 帧、
再记录 50 个变化帧。CDP 往返提供每个 Browser 的 P50/P95；Chromium trace
在 `viz` 类别记录请求、结果、额外重绘请求、共享槽映射与 `readPixels`。
...
此探针不经过 Node，不能据它量化正式导出宿主的开销。测试页把 640×360 Canvas 用 CSS 拉伸到
